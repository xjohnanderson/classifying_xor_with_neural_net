...
A lightweight, from-scratch implementation of a Feedforward Neural Network using **NumPy**. This project demonstrates the core mechanics of deep learning, including **forward propagation**, **backpropagation**, and **gradient descent**.
...
The network relies on the following key components:

1. **Feedforward:** Calculations for each layer:
...
* `__init__`: Initializes weights with a standard normal distribution and biases to zero.
* `feedforward`: Passes input through the hidden layer to the output.
* `backward`: Calculates gradients and updates weights using the learning rate.
* `train`: The main loop coordinating the forward and backward passes.
